...
window.addEventListener("wheel", function (e) {
    ...
    if (e.deltaY > 0) {
        ...
        if (page === 3) {
            ...
            setTimeout(() => {
                const background_2 = document.querySelector('.section_2box.background_2');
                if (background_2) { // 요소가 존재하는지 확인
                    background_2.style.display = "flex"; // flex를 문자열로 작성
                }


                setTimeout(() => {
                    const background_3 = document.querySelector('.section_2box.background_3');
                    if (background_3) { // 요소가 존재하는지 확인
                        background_3.style.display = "flex"; // flex를 문자열로 작성
                    }
                }, 4000);
        ...
        */
        ...
        if (page === 3) {
            ...
            setTimeout(() => {
                const background_3 = document.querySelector('.section_2box.background_3');
                if (background_3) { // 요소가 존재하는지 확인
                    background_3.style.display = "none"; // flex를 문자열로 작성
                }
                ...
                setTimeout(() => {
                    const background_2 = document.querySelector('.section_2box.background_2');
                    if (background_2) { // 요소가 존재하는지 확인
                        background_2.style.display = "none"; // flex를 문자열로 작성
                    }
                }, 4000);
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Set initial slide width for smooth scrolling
    const slideWidth = swiperLi[0].offsetWidth;
    swiperUl.style.width = `${slideWidth * totalSlides}px`;
    ...
    buttonNext.addEventListener('click', () => {
        if (currentIndex < totalSlides - 1) {
            currentIndex++;
            swiperUl.style.transform = `translateX(-${slideWidth * currentIndex}px)`;
            updateProgressBar();
        }
    });
